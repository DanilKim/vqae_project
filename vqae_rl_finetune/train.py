...
def train(model, train_loader, eval_loader, num_epochs, output, rl=False, temperature=1.0):
    ...
    if rl:
        ...
        opt = {'cider_reward_weight':1, 'bleu_reward_weight':0}
        init_scorer('vqae-all-idxs')
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, c, l) in enumerate(train_loader):
            ...
            if rl:
                gen_result, sample_logprobs = model.explain_sample(v, q, opt={'sample_max':0})
                reward = get_self_critical_reward(model, v, q, c[:, 1:], gen_result, opt)
                vqe_loss = rl_criterion(sample_logprobs, gen_result.data, torch.from_numpy(reward).float().cuda())
            else:
