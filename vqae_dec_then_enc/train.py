...
def train(model, train_loader, eval_loader, num_epochs, output, dataset, gt_train=False):
    ...
    best_eval_score = 0
    print_freq = 10
    A, B = evaluate(model, eval_loader, dataset, gt_train)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, c, l, f) in enumerate(train_loader):
            ...
            if dataset == 'VQA2':
                f = Variable(f[sort_ind]).cuda().view(bs, 1).float()

            ans_pred, exp_pred = model(v, q, c[:, :-1], f, [j-1 for j in l], ml, gt_train)
            if i % 100 == 0:
        ...
        model.train(False)
        eval_score, bound = evaluate(model, eval_loader, dataset, gt_train)
        model.train(True)
...
def evaluate(model, dataloader, dataset, gt_train=False):
    score = 0
    upper_bound = 0
    num_data = 0
    results = []
    ...
    for i, (v, q, a, c, l, f) in enumerate(dataloader):
        ...
        if dataset == 'VQA2':
            f = Variable(f).cuda().view(batch_size, 1).float()

        if gt_train:
            c = Variable(c, volatile=True).cuda()
            ans_pred, exp_pred = model.evaluate(v, q, f, c)
            c = c.data
        else:
