...
class ExplainEmbedding(nn.Module):
    ...
    def forward(self, dec_hidden, x):
        # x: [batch, sequence, in_dim]
        batch = x.size(0)
        #hidden = self.init_hidden(batch)
        self.rnn.flatten_parameters()
        output, hidden = self.rnn(x.detach(), dec_hidden.unsqueeze(0))
...
class STDecoder(nn.Module):
    ...
    def forward(self, joint, captions, lengths, max_len):
        ...
        sampled_ids = torch.cat(sampled_ids, 1)
        return torch.cat([_.unsqueeze(1) for _ in outputs], 1), sampled_ids.squeeze(), last_hidden
    ...
    def tf_sample(self, joint, gt, lengths, max_len=18):
        # greedy sample
        l = torch.Tensor(lengths)
        time_step = max_len-1
        batch_size = joint.size(0)
        ...
        for step in range(time_step):
            it = gt[:, step].clone()
            if step >= 1 and gt[:, step].data.sum() == 0:
                break
            if (step == l-1).sum() > 0:
                last_idx = (step == l-1).nonzero().squeeze().cuda()
                last_hidden[last_idx] = h0[last_idx]
            inputs = self.embed(it)
            h0, c0 = self.lstm_cell(inputs, (h0, c0))
            output = self.fc_out(h0)
            outputs.append(output)
            predicted = output.max(1)[1] * word_flag.long()
