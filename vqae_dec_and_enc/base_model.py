...
class VQAE(nn.Module):
    ...
    def forward(self, v, q, gt, lengths, max_len):
        ...
        vqe_logits, _ = self.generator(joint_repr, gt, lengths, max_len)
    ...
    def evaluate(self, v, q):
        ...
        joint_repr = q_repr * v_repr
        vqa_logits = self.classifier(joint_repr)
        vqe_pred, _ = self.generator.sample(joint_repr)
...
class VQAE2(nn.Module):
    ...
    def forward(self, v, q, gt, lengths, max_len):
        ...
        joint_repr = q_repr * v_repr
        #vqa_logits = self.classifier(joint_repr)
        vqe_logits, e_emb = self.generator(joint_repr, gt, lengths, max_len)
        vqa_logits = self.classifier(e_emb)
    ...
    def evaluate(self, v, q):
        ...
        joint_repr = q_repr * v_repr
        #vqa_logits = self.classifier(joint_repr)
        vqe_pred, e_emb = self.generator.sample(joint_repr)
        vqa_logits = self.classifier(e_emb)
