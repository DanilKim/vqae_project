...
def _load_VQA2E(dataroot, name, answer_dir, img_id2val):
    ...
    #answer_path = os.path.join(dataroot, answer_dir, '%s_target_VQA2.pkl' % name)       # VQA on VQA vocab
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, question_dict, caption_dict, answer_dir, dataroot='data'):
        ...
        #self.entries = _load_dataset(dataroot, name, answer_dir, self.img_id2idx, thres)
...
class VQAEVQA2Dataset(Dataset):
    def __init__(self, name, question_explain_dict, answer_dir, dataroot='data'):
...
class VQAEDataset(Dataset):
    def __init__(self, name, question_dict, explanation_dict, answer_dir, dataroot='data', result=False):
        ...
        self.num_map = {'1': 'one', '2': 'two', '3': 'three', '4': 'four', '5': 'five', '6': 'six', '7': 'seven', '8': 'eight', '9': 'nine', '10': 'ten'}
        self.entries = []
        for explain, answer in zip(explains, answers):
            ...
            #    masked_explain = self.mask_sentence(explain['explanation'][0].lower(),
            #                                        self.label2ans[answer['labels'][np.argmax(np.array(answer['scores']))]])
            self.entries.append({'question_id' : answer['question_id'],
                                 ...
                                 'answer'      : answer,
                                 #'explanation' : masked_explain})
                                 'explanation' : explain['explanation'][0]})

        #self.explanation_dictionary.add_word('<mask>')
        self.tokenize()
        self.tensorize()
        self.v_num = self.features.size(1)
        self.v_dim = self.features.size(2)
    ...
    def mask_sentence(self, sentence, answer):
        if answer in self.num_map:
            mapped_answer = self.num_map[answer]
            return sentence.replace(mapped_answer, '<unk>')
        else:
            return sentence.replace(answer, '<unk>')
    ...
    def tokenize(self, max_qu_length=14, max_exp_length=18):
        ...
        for entry in self.entries:
            ...
            #etokens.extend(random.sample(range(3, len(self.explanation_dictionary.word2idx)),
            #                             len(self.explanation_dictionary.tokenize(entry['explanation'], False))))
            etokens.append(self.explanation_dictionary.word2idx['<end>'])
            etokens = etokens[:max_exp_length]
            #random.shuffle(etokens)
            if len(qtokens) < max_qu_length:
                # Note here we pad in front of the sentence
                qpadding = [self.question_dictionary.padding_idx] \
                          * (max_qu_length - len(qtokens))
...
class VQAXDataset(Dataset):
    def __init__(self, name, question_dict, explanation_dict, answer_dir, dataroot='data', result=False):
        super(VQAXDataset, self).__init__()
        ...
        qid2q = {q['question_id'] : q['question'] for q in questions}

        explain_path = os.path.join(dataroot, 'VQA-X/textual/%s_exp_anno.json' % name)
        explains = json.load(open(explain_path))

        explains = sorted(explains.items(), key=lambda x:int(x[0]))
        ...
        self.entries = []
        for explain, answer in zip(explains, answers):
            utils.assert_eq(int(explain[0]), answer['question_id'])
            img_id = answer['image_id']
            #if len(answer['labels']):
            #    masked_explain = self.mask_sentence(explain['explanation'][0].lower(),
            #                                        self.label2ans[answer['labels'][np.argmax(np.array(answer['scores']))]])
            for i in range(len(explain[1])):
                self.entries.append({'question_id' : answer['question_id'],
                                     'image_id'    : answer['image_id'],
                                     'image'       : self.img_id2idx[img_id],
                                     'question'    : qid2q[answer['question_id']],
                                     'answer'      : answer.copy(),
                                     #'explanation' : masked_explain})
                                     'explanation' : explain[1][i]})
    ...
    def tokenize(self, max_qu_length=14, max_exp_length=14):
        """Tokenizes the questionse
    ...
    def tensorize(self):
        ...
        for entry in self.entries:
            ...
            answer = entry['answer']
            if type(answer['labels']) is not list:
                pdb.set_trace()
            labels = np.array(answer['labels'])
            scores = np.array(answer['scores'], dtype=np.float32)
            if len(labels):
...
def data_factory(data, split, q_dict, c_dict, t_dir):
    if data == 'VQAE':
        dset = VQAEDataset(split, q_dict, c_dict, t_dir)
    elif data == 'VQAX':
        dset = VQAXDataset(split, q_dict, c_dict, t_dir)
    elif data == 'VQA2':
        dset = VQAFeatureDataset(split, q_dict, c_dict, t_dir)
    elif data == 'VQAEVQA2':
        dset = VQAEVQA2Dataset(split, q_dict, c_dict, t_dir)
    else:
        raise ValueError
