...
def compute_target(answers_dset, explains_dset, ans2label, name, cache_root='../data/cache/VQA2_2_VQAX'):
    ...
    ***answers_dset should be preprocessed***
    ...
    """
    target = []
    qid2ans = {ans['question_id'] : ans for ans in answers_dset}
    for qi in explains_dset:
        ans_entry = qid2ans[int(qi)]
        answers = ans_entry['answers']
        answer_count = {}
...
if __name__ == '__main__':
    train_exp_file = '../data/VQA-X/textual/train_exp_anno.json'
    train_explains = json.load(open(train_exp_file))

    val_exp_file = '../data/VQA-X/textual/val_exp_anno.json'
    val_explains = json.load(open(val_exp_file))
    ...
    question2qid = questionDict(questions)
    occurence = filter_answers(answers, 9)
    ans2label = create_ans2label(occurence, 'trainval', '../data/cache/VQA2_2_VQAX')
    compute_target(train_answers, train_explains, ans2label, 'train', '../data/cache/VQA2_2_VQAX')

    #val_answers = json.load(open('../data/v2_mscoco_val2014_annotations.json'))['annotations']
    compute_target(val_answers, val_explains, ans2label, 'val', '../data/cache/VQA2_2_VQAX')
