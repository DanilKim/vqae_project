...
class ExplainEmbedding(nn.Module):
    ...
    def sample_gumbel(self, logits, temperature=1.0):
        ...
        x = torch.matmul(x, self.emb.weight.detach())
        x = self.dropout(x)
        return x
...
class STDecoder(nn.Module):
    ...
    def forward(self, joint, captions, lengths, max_len):
        ...
        for i in range(time_step):
            ...
            output = self.fc_out(self.fc_dropout(h0))
            outputs.append(output)
            predicted = output.max(1)[1] * Variable((i < l-1).long().cuda())
            max_ids.append(predicted.unsqueeze(1))

        max_ids = torch.cat(max_ids, 1)
        return torch.cat([_.unsqueeze(1) for _ in outputs], 1), max_ids
